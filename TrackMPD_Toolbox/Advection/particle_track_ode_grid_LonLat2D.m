function [ ln, lt, ts ] = particle_track_ode_grid_LonLat2D(Lon, Lat, ux, uy, Coordinates, ...
                                                  varargin )
...
if strcmpi(Coordinates,'spherical') 
    ...
    % Convert values in ux and uy (in cm/s) to dLon/dDay and dLat/dDay,
    % respectively
   
    ux = ux .* 86400 .* repmat(LonPerCM,[1,1,size(ux,3)]); %to dLon/dDay
    uy = uy .* 86400 .* repmat(LatPerCM,[1,1,size(uy,3)]); %to dLat/dDay
...
elseif strcmpi(Coordinates,'cartesian')
    ...
    % Convert values in ux and uy (in cm/s) to dM/dDay
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[ln,lt,ts] = particle_track_ode_grid2D( Lon, Lat, ux, uy, varargin{:} );
