function transformMARCinputs_2D(conf_name,confOGCM_name)
% TRANSFORM OGCM outputs (MARC-2D: MARS online) to TrackMPD format (2D mode)
% I.Jalon-Rojas  22 June 2020
...
MARC_Prefix = conf.OGCM.MARC_Prefix;
MARC_Suffix = conf.OGCM.MARC_Suffix;
fnames=getAllFiles(conf.OGCM.BaseDir,strcat(MARC_Prefix,'*',MARC_Suffix),true);
...
for n=1:nFiles %Loop to read MARC files
...
timestamps(n)=ncread(file,'time')/(3600*24)+datenum(1900,1,1,0,0,0);
...
% read velocity and elevation at each time step
    ...
    u = ncread(file,'U'); %,[1 1 1 n],[Inf Inf Inf 1]);      % units = 'm/s'
    v = ncread(file,'V'); %,[1 1 1 n],[Inf Inf Inf 1]);      % units = 'm/s'
...
% transform velocities
    ...
    % c) interpolation at lat lon
     ...
     u=griddata(lat_u(1,:),lon_u(:,1),u,Lat',Lon); 
     v=griddata(lat_v(1,:),lon_v(:,1),v,Lat',Lon);
