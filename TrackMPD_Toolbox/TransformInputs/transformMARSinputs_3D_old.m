function transformMARSinputs_3D(conf_name)
...
% transform velocities
    ...
    for lv=1:numlvl
        u(:,:,lv)=griddata(lat_u(1,:),lon_u(:,80),U(:,:,lv),Lat',Lon); 
        v(:,:,lv)=griddata(lat_v(1,:),lon_v(:,80),V(:,:,lv),Lat',Lon);
    end
...
% create depths at each grid point
    ...
    for i=1:length(ZZ)
        depth(:,:,i)=ZZ(i)*(BottomDepth+E); %+E-E; Tranform: sigma*(BottomDepth+E)+E 
