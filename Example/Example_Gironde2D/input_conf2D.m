function conf = input_conf2D(varargin)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Inputs data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%

conf.Data.BaseDir = fullfile(pwd,'InputData2D'); %Folder containing inputs
conf.Data.Domain = fullfile(conf.Data.BaseDir,'coastline_Gironde.txt'); % domain file (full path)
conf.Data.ParticlesFile = fullfile(conf.Data.BaseDir,'initial_particle_location_4_2D.csv'); % particle release file (full path)
...
conf.Traj.Mode = '2D'; %'2D' or '3D' tracking
...
conf.Traj.ScenarioName = [conf.OGCM.DomainName '_test2D']; % Free choice
...
if strcmpi(conf.Traj.Resuspension,'yes')
  conf.Traj.ResOption = 'soulsby'; % Options: soulby, waldschlager
